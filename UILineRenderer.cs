...
public class UILineRenderer : Graphic
{
	...
	void Start ()
	{
		Points = new Vector2[Screen.width / 10];
	...
	IEnumerator Count ()
	{
		...
		while (true) {
			for (int i = 0; i < Points.Length; i++) {
				Points [i] = new Vector2 (Mathf.Abs (Ancla.transform.position.x) % Screen.width, Mathf.Abs (Ancla.transform.position.z) % Screen.height);
